...
const AdminBusinessEdit = () => {
    ...
    const [business, setBusiness] = useState({
      ...
      const [isBusinessUpdatedDialogOpen, setBusinessUpdatedDialogOpen] = useState(false);
      ...
      const fetchBusinessData = async (busId) => {
        try {
          const response = await fetch(`http://localhost:8080/business/getInfoById/${busId}`);
      ...
      const handleConfirmUpdate = async () => {
        try {
          const response = await fetch(`http://localhost:8080/business/updateBusiness/${id}`, {
            method: 'PUT',
            headers: {
            ...
            <Grid item xs={9} style={{ backgroundColor: '#ffffff' }}>
              <div className="importantannouncementb2">
                <div className='importantannouncement-boxb2'>
                    <h1 style={{ color: "#fff", marginLeft: '10px' }}>EDIT BUSINESS</h1>
                </div>
              ...
              <Dialog open={isDialogOpen} onClose={() => setDialogOpen(false)} PaperProps={{ style: { backgroundColor: '#ffffff' } }}>
                ...
                <Dialog open={isBusinessUpdatedDialogOpen} onClose={handleCloseBusinessUpdatedDialog} PaperProps={{ style: { backgroundColor: '#ffffff' } }}>
                  <DialogTitle style={{ margin: 'auto', textAlign: 'center', color: '#16558f', fontWeight: 'bolder', fontSize: '40px' }}>
                    Updated
                  </DialogTitle>
                  <p style={{fontSize: "23px", color: '#000000', textAlign: 'center', marginTop: "0px"}}>Your business is successfully updated!</p>
                  ...
                  <DialogActions>
                    ...
                    <Button onClick={handleViewBusinessButtonClick} 
