...
const EmergencyAlertList = () => {
  ...
  const fetchHotlineData = async () => {
    try {
      ...
      // Filter out hotlines with isdelete set to 1
      setHotlines(data.filter((hotline) => hotline.isdelete !== 1));
    } catch (error) {
      console.error(`Error fetching hotline data: ${error.message}`);
    }
  };
  ...
  const AlertBox = ({ typeOfIncident, date, location, detailedIncident }) => (
    <div className="alerts">
      ...
      <div>Type of Incident: {typeOfIncident}</div>
      <div>Date: {date}</div>
      <div>Location: {location}</div>
      <div>Detailed Incident: {detailedIncident}</div>
    </div>
  );
